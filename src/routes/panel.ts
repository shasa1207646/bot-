...
router.post('/panel/register', async (req: Request, res: Response) => {
  ...
  try {
    ...
    // Cast to TEXT explicitly — guards against legacy DBs where this column is BIGINT
    await pool.query(
      `UPDATE oauth_states SET telegram_user_id=$1::TEXT WHERE state=$2`,
      [pendingId, state]
    );
...
router.get('/panel/auth/callback', async (req: Request, res: Response) => {
  ...
  try {
    ...
    // FIX: проверяем pending ДО вычисления redirectPage
    if (!pending.rows[0]) return res.redirect(`/panel/mod?panel_error=pending_not_found`);
    const pendingUser = pending.rows[0];
    const redirectPage = pendingUser.panel_role === 'admin' ? '/panel/admin' : '/panel/mod';
